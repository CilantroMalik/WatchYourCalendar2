...
struct WatchConfig: View {
    ...
    @State var firstLunch: [Int] = [3, 3, 3, 3, 3, 3]
    ...
    func sendToWatch() {
        ...
        for i in 0...5 {
            ...
            sports[i+1] = sportsArr[i] == "" ? "Go Home!" : sportsArr[i]
        }
        let a = classArr[0]
        let b = classArr[1]
        let c = classArr[2]
...
//        UserDefaults.standard.set(sportsArr, forKey: "sports")
        ...
        let uinfo = UserScheduleInfo(classes: classes, ZLunch: ZLunch, sports: sports)
        let encoder = JSONEncoder()
        let dat = try! encoder.encode(uinfo)
